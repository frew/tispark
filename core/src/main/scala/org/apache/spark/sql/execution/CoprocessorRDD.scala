...
case class ColumnarRegionTaskExec(
  ...
  def fetchTableResultsFromHandles(
    ...
    iter.flatMap { row =>
      ...
      val indexTaskRanges = indexTasks.asScala.flatMap {
        _.getRanges.asScala
      }
        ...
        while (queue.nonEmpty) {
          ...
          if (isTaskRangeSizeInvalid(front)) {
            ...
            front.getRanges
              ...
              .foreach(range => {
                queue += RegionTask.newInstance(front.getRegion, front.getStore, range.asJava)
              })
          } else {
      ...
      def doIndexScan(): Unit =
        while (handleIdx < handles.size) {
          ...
          indexTasks.asScala.foreach { task =>
      ...
      def doDowngradeScan(taskRanges: List[KeyRange]): Unit = {
        ...
        val downgradeTasks = RangeSplitter
          .newSplitter(session.getRegionManager)
          .splitRangeByRegion(
            KeyRangeUtils.mergeSortedRanges(taskRanges.asJava),
            dagRequest.getStoreType)

        downgradeTasks.asScala.foreach { task =>
          ...
          submitTasks(downgradeTasks.asScala.toList, downgradeDagRequest)
        }
      }
      ...
      if (satisfyDowngradeThreshold) {
        ...
        logger.info(
          s"Index scan task range size = ${indexTaskRanges.size}, " +
            ...
            s"index scan handle size = ${handles.size}, will try to merge.")
